...
Because the pipeline is unattended, reliability work matters more than features:

- **Idempotency guard** — the job checks for today's post and campaign before writing anything,
  so a retry can never double-publish or double-send.
- **Health diagnostics** — distinguishes "fired on time, failed instantly" (credentials) from
  "interrupted mid-run" (host went down). Two symptoms a scheduler reports identically.
- **Resilient integrations** — the email API's documented endpoint returns 404 on this account;
  campaign updates go through an alternate path that preserves template tokens.
- **Content verification** — every image is visually inspected before captioning, after a
  mislabelled file was nearly published under a caption describing something else entirely.

Detailed design decisions, failure handling and architecture: **[ENGINEERING.md](ENGINEERING.md)**
